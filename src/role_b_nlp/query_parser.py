...
from src.role_b_nlp.object_matcher import extract_object_keywords

def extract_sub_events(text: str) -> List[str]:
    """
    Tách các sự kiện con cho TRAKE dưới dạng List[str] thực dụng.
    """
    if not text:
        return []

    # Pattern 1: (1) ... (2) ... hoặc 1. ... 2. ... hoặc bước 1: ...
    pattern_numbered = re.split(r'(?:\(\d+\)|\b\d+[\.\)]|\bbước\s+\d+:?)', text, flags=re.IGNORECASE)
    parts = [normalize_text(p) for p in pattern_numbered if normalize_text(p)]
    
    if len(parts) > 1:
        return parts

    # Pattern 2: Từ nối thời gian ("sau đó", "tiếp theo", "rồi")
    pattern_transitions = re.split(r'\b(?:sau đó|tiếp theo|rồi|cuối cùng)\b', text, flags=re.IGNORECASE)
...
def parse_query(query: Query) -> ParsedQuery:
    ...
    target_objs = extract_object_keywords(query.raw_text)
    if target_objs:
